...
class TestFurnitureRoutes:
    ...
    def test_get_all_furniture_no_filters(self, mock_get_all, client):
        ...
        The dummy furniture is returned and the response should include the furniture details.
        """
    ...
    def test_get_all_furniture_with_filters(self, mock_search, client, query, param, expected_quantity):
        """
        Test GET /api/furniture with either a name or type filter.
        
        The mocked search returns an item with the given quantity.
        """
        dummy_item = {"furniture": create_dummy_furniture(), "quantity": expected_quantity}
    ...
    def test_get_furniture_by_id_found(self, mock_get_quantity, mock_get_furniture, client):
        """
    ...
    @patch("app.routes.get_authenticated_user", side_effect=AuthenticationError("Unauthorized"))
    ...
    def test_add_furniture_table(self, mock_table, mock_add_furniture, mock_auth, client):
        """
        ...
        dummy_table.to_dict.return_value = {"name": "Table", "price": 200, "description": "A round table"}
    ...
    def test_add_furniture_sofa(self, mock_sofa, mock_add_furniture, mock_auth, client):
        ...
        dummy_sofa.to_dict.return_value = {"name": "Sofa", "price": 300, "description": "A comfy sofa"}
    ...
    def test_add_furniture_bed(self, mock_bed, mock_add_furniture, mock_auth, client):
        ...
        mock_auth.return_value = dummy_user()
        dummy_bed = MagicMock()
        dummy_bed.to_dict.return_value = {"name": "Bed", "price": 400, "description": "A queen bed"}
    ...
    def test_add_furniture_bookcase(self, mock_bookcase, mock_add_furniture, mock_auth, client):
        """
        ...
        dummy_bookcase.to_dict.return_value = {"name": "Bookcase", "price": 150, "description": "A bookcase"}
    ...
    @patch("app.routes.get_authenticated_user")
    @patch("app.routes.inventory.add_furniture")
    def test_add_furniture_generic_exception(self, mock_add_furniture, mock_auth, client):
        """
    ...
    def test_update_furniture_quantity_success(self, mock_update, mock_auth, client):
        """
        Test PUT /api/furniture/<furniture_id> to update furniture quantity successfully.
    ...
    @patch("app.routes.get_authenticated_user")
    @patch("app.routes.inventory.update_quantity")
    def test_update_furniture_quantity_generic_exception(self, mock_update, mock_auth, client):
        """
        Test PUT /api/furniture/<furniture_id> where update_quantity raises a generic Exception.
    ...
    def test_remove_furniture_auth_error(self, client, mocker):
        ...
        Should return a 401 status code.
        """
        mocker.patch("app.routes.get_authenticated_user", side_effect=AuthenticationError("Auth error"))
        response = client.delete("/api/furniture/123")
        assert response.status_code == 401
        data = response.get_json()
        assert "Auth error" in data["error"]
    ...
    def test_remove_furniture_generic_exception(self, client, mocker):
        """
        Test DELETE /api/furniture/<furniture_id> where removal raises a generic Exception.
        
        Should return a 500 status code.
        """
        dummy = dummy_user()
        mocker.patch("app.routes.get_authenticated_user", return_value=dummy)
        mocker.patch("app.routes.inventory.remove_furniture", side_effect=Exception("Generic error"))
        response = client.delete("/api/furniture/123")
        assert response.status_code == 500
        data = response.get_json()
        assert "Generic error" in data["error"]
...
class TestUserRoutes:
    """Tests for the user-related routes (registration, login, profile, password, logout)."""
    ...
    @patch("app.routes.user_manager.login", side_effect=AuthenticationError("Invalid credentials"))
    ...
    def test_update_user_profile_success(self, mock_update, mock_auth, client):
        ...
        mock_auth.return_value = dummy
        mock_update.return_value = True
        payload = {"full_name": "New Name", "email": "new@example.com", "shipping_address": "New Address"}
    ...
    @patch("app.routes.get_authenticated_user")
    @patch("app.routes.user_manager.update_user")
    def test_update_user_profile_generic_exception(self, mock_update, mock_auth, client):
        """
    ...
    def test_refresh_token_auth_error(self, mock_refresh, client):
        """
        Test POST /api/users/refresh-token when refresh_access_token raises AuthenticationError.
    ...
    def test_refresh_token_generic_exception(self, mock_refresh, client):
        """
        Test POST /api/users/refresh-token when refresh_access_token raises a generic Exception.
    ...
    def test_get_user_profile_generic_exception(self, client, mocker):
        """
        Test GET /api/users/profile where an exception occurs while accessing user attributes.
    ...
    def test_update_password_missing_fields(self, mock_auth, client):
        ...
        response = client.put("/api/users/password", json={"current_password": "oldpass"})
        assert response.status_code == 400
        data = response.get_json()
        assert "Missing current or new password" in data["error"]
...
class TestCartRoutes:
    ...
    def test_find_and_add_to_cart_success(self, mock_find_and_add, mock_auth, client):
        """
        Test POST /api/cart/find-and-add successfully finds and adds an item to the cart.
        ...
        assert response.status_code == 200
        mock_find_and_add.assert_called_with(dummy.shopping_cart, "chair", 1, color="red")
    ...
    # Exception branches for /cart/add, /cart/find-and-add, /cart/remove, /cart/clear, /cart/discount
    ...
    def test_add_to_cart_generic_exception(self, mock_auth, client):
        ...
        with patch.object(inventory, "get_furniture", side_effect=Exception("Generic error")):
            payload = {"furniture_id": "furn1", "quantity": 1}
            response = client.post("/api/cart/add", json=payload)
            assert response.status_code == 500
            data = response.get_json()
    ...
    def test_find_and_add_generic_exception(self, mock_auth, client):
        ...
        with patch.object(cart_locator, "find_and_add_to_cart", side_effect=Exception("Generic error")):
            payload = {"type": "chair", "quantity": 1, "color": "red"}
            response = client.post("/api/cart/find-and-add", json=payload)
            assert response.status_code == 500
            data = response.get_json()
    ...
    def test_remove_from_cart_value_error(self, mock_auth, client):
        """
        Test DELETE /api/cart/remove/<furniture_id> with non-numeric quantity causing a ValueError.
    ...
    def test_remove_from_cart_generic_exception(self, mock_auth, client):
        """
        Test DELETE /api/cart/remove/<furniture_id> where remove_item raises a generic Exception.
    ...
    def test_apply_discount_value_error(self, mock_auth, client):
        """
        Test POST /api/cart/discount with non-numeric discount value causing a ValueError.
        ...
        assert response.status_code == 400
        data = response.get_json()
        assert "could not convert" in data["error"] or "invalid literal" in data["error"]
...
class TestCheckoutOrdersRoutes:
    ...
    def test_process_checkout_valid(self, mock_payment_method, mock_checkout, mock_auth, client):
        """
...
@pytest.mark.parametrize(
    "auth_exception, payment_exception, checkout_exception, expected_status, expected_error_substring",
    [
...
def test_process_checkout_exceptions(
    ...
    Depending on which exception is raised in authentication, payment method, or checkout processing,
    the endpoint should return the expected status code and error message.
    """
    if auth_exception:
...
def test_get_authenticated_user_valid():
    ...
    with app.test_request_context("/", headers={"Authorization": "Bearer validtoken"}):
        ...
        with patch("app.routes.user_manager.authenticate_with_token", return_value=dummy_user_obj) as mock_auth:
...
def test_get_authenticated_user_invalid_header():
    ...
    """
    app = Flask(__name__)
    with app.test_request_context("/", headers={"Authorization": "InvalidToken sometoken"}):
        with pytest.raises(AuthenticationError) as excinfo:
            get_authenticated_user()
        assert "Missing or invalid Authorization header" in str(excinfo.value)
...
def test_get_furniture_by_id_exception(client, mocker):
    """
    Test GET /api/furniture/<furniture_id> when inventory.get_furniture raises an exception.
    
    Should return a 500 status code.
    """
    mocker.patch("app.routes.inventory.get_furniture", side_effect=Exception("Test error"))
    response = client.get("/api/furniture/123")
    assert response.status_code == 500
    data = response.get_json()
...
def test_get_order_details_exception(client, mocker):
    ...
    dummy = dummy_user()
    mocker.patch("app.routes.get_authenticated_user", return_value=dummy)
    mocker.patch("app.routes.order_manager.get_order", side_effect=Exception("Test error"))
    response = client.get("/api/orders/someorder")
    assert response.status_code == 500
    data = response.get_json()
    assert "Test error" in data["error"]
...
def test_get_user_orders_exception(client, mocker):
    ...
    dummy = dummy_user()
    mocker.patch("app.routes.get_authenticated_user", return_value=dummy)
    mocker.patch("app.routes.order_manager.get_user_orders", side_effect=Exception("Test error"))
    response = client.get("/api/orders")
    assert response.status_code == 500
    data = response.get_json()
    assert "Test error" in data["error"]
